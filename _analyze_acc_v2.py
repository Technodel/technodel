...
STOPWORDS = {
    ...
    "was", "were", "this", "that", "these", "those", "it", "its", "up", "down", 
    "under", "over", "etc", "i", "can", "only", "about", "be", "has", "have", "had", "do", "does", "did", "as", 
    "if", "then", "else", "not", "no", "yes", "some", "any", "all", "each", "every", "other", "such", 
...
def analyze():
    ...
    for i in range(0, len(top_120), 5):
        print(", ".join([f"{k} ({v})" for k, v in top_120[i:i+5]]))
    ...
    for group, pattern in patterns.items():
        ...
        matches = [t for t in products if regex.search(t)]
